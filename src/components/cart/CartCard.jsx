...
const CartCard = ({ product }) => {
  const { bookId, image, authors, price, nameBook } = product;
  const wishlisted = null;
        ...
        <div className="absolute bottom-0 items-center space-y-2 text-gray-100 right-16 sm:hidden">
          ...
          >
            <svg
              ...
              strokeWidth={2}
              stroke="currentColor"
              className={`w-8 p-2 m-auto ${
                wishlisted ? "fill-pink-500" : "hover:fill-current"
              } bg-pink-100 opacity-80 rounded-full h-8`}
            >
              <path
                strokeLinecap="round"
        ...
        <div className="flex flex-col justify-between mt-4 sm:space-y-6 sm:mt-0 sm:block">
          ...
          <div className="relative items-center hidden  space-x-4 text-gray-100 sm:flex">
            ...
            <button
              ...
              disabled={wishlisted}
              // onClick={(e) => moveToWishlistHandler(product)}
              className="absolute right-8  w-12 h-12 p-0 m-0 text-pink-500 rounded-full"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                ...
                className={`w-8 p-2 m-auto ${
                  wishlisted ? "fill-pink-500" : "hover:fill-current"
                }  border border-gray-700  bg-gray-800 rounded-full h-8`}
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
